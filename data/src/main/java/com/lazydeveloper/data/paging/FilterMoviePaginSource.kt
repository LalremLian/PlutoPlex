...
class FilterMoviePagingSource(
...
) : PagingSource<Int, MovieFilterResponse.Result>() {

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, MovieFilterResponse.Result> {
        return try {
            val nextPage = params.key ?: 1
            val response = appRepo.filterMovie(
                nextPage = nextPage,
                year = year.toString(),
    ...
    override fun getRefreshKey(state: PagingState<Int, MovieFilterResponse.Result>): Int? {
